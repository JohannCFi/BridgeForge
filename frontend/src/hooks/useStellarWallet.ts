// ============================================================
// BridgeForge – Stellar Wallet Hook
// Connects via Freighter browser extension.
// Burn = send tokens back to the issuer (Payment op).
// ============================================================
...
import type { ChainWallet, BurnParams } from "./useWallet";

const ASSET_CODE = "tEURCV";
const NETWORK_PASSPHRASE = "Test SDF Network ; September 2015";
const HORIZON_URL = "https://horizon-testnet.stellar.org";

export function useStellarWallet(): ChainWallet {
  ...
  const connect = useCallback(async () => {
    ...
    // Check if site is allowed, if not request access (opens Freighter popup)
  ...
  const signBurn = useCallback(
    async (params: BurnParams): Promise<string> => {
      ...
      // Build the payment transaction via Horizon
  ...
  return { address, connected, connect, disconnect, signBurn };
}
